...
const localKeypair = anchor.web3.Keypair.fromSecretKey(Buffer.from(JSON.parse(fs.readFileSync("/home/myware/.config/solana/nft_wallet.json", {encoding: "utf-8",}))));
// const dev_wallet_pubkey = new anchor.web3.PublicKey("DWb5SSsbhorqjud6EkdBwHMLSAFfun921sp2xfH9oY4a");
...
const cache_path = "./devnet_tests/cache/"
const holders_fname = 'top_holder_addresses.csv'
...
async function airdrop_nft (receiver_address, nft_mint_address, nft_token_account) {
    ...
    let receiverAta = await tboMint.getOrCreateAssociatedAccountInfo(receiver_pubkey);
    console.log("loaded receiver ata: ", receiverAta.address.toString());
...
async function loop_airdrop_nfts () {
    ...
    while (destination_addresses.length > 0){
        ...
        try {
            ...
            console.log("transaction success, updated record with sig: "+ tx_sig + " now sleeping 10 seconds...");
